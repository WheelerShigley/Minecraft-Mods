...
public class Registrations {
    ...
    private static final Predicate<ServerCommandSource> isServerOrOperator = (source) -> {
        ...
        return server.getPlayerManager().isOperator(
            sourcePlayer.getPlayerConfigEntry()
        );
    };
    ...
    public static void registerCommands() {
        ...
        Command<ServerCommandSource> externalBalanceCommand = (context) -> {
            ...
            MinecraftServer server = sourcePlayer.getEntityWorld().getServer();
            Collection<PlayerConfigEntry> requestedPlayers = GameProfileArgumentType.getProfileArgument(context, "target");

            ServerPlayerEntity targetPlayer;
            if(server == null) {
                return 0;
            }
            for(Iterator<PlayerConfigEntry> configEntryIterator = requestedPlayers.iterator(); configEntryIterator.hasNext(); ) {
                PlayerConfigEntry currentRequestedPlayer = configEntryIterator.next();
                targetPlayer = server.getPlayerManager().getPlayer( currentRequestedPlayer.id() );
        ...
        Command<ServerCommandSource> paymentCommand = (context) -> {
            ...
            ServerPlayerEntity targetPlayer = server.getPlayerManager().getPlayer(
        ...
        Command<ServerCommandSource> tradeExperienceCommand = (context) -> {
            ...
            PermissionLevel playerPermissionLevel = server.getPermissionLevel( player.getPlayerConfigEntry() ).getLevel();
            if( playerPermissionLevel.isAtLeast(PermissionLevel.MODERATORS) ) {
                messageBuilder.append(
                    Text.translatable("trade_experience.command.text.reloaded").getString()
                );
            } else {
                messageBuilder.append(
                    Text.translatable("trade_experience.command.text.insufficient_permission").getString()
                );
                return 0;
            }
