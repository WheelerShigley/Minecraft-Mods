...
//    );
    ...
    public ItemStack process(ItemStack stack, LootContext context) {
        ...
        SilkTouchPlus.LOGGER.info(String.valueOf(nbtElement.getNbtType()));

        MutableObject<NbtCompound> mutableObject = new MutableObject<>();
        TypedEntityData<BlockEntityType<?>> blockEntityDataComponent = stack.get(DataComponentTypes.BLOCK_ENTITY_DATA);
        if (blockEntityDataComponent == null) {
            return stack;
        }
        Supplier<NbtElement> supplier = () -> {
            if (mutableObject.getValue() == null) {
                mutableObject.setValue(
                        blockEntityDataComponent.copyNbtWithoutId()
                );
            }
